...
UPDATE order_status_history SET order_status = 'CONFIRMED'
WHERE order_status IN ('PREPARING', 'READY', 'IN_TRANSIT', 'DELIVERING')
AND order_status IS NOT NULL;

UPDATE order_status_history SET order_status = 'CANCELLED'
WHERE order_status = 'FAILED'
AND order_status IS NOT NULL;

UPDATE order_status_history SET order_status = 'PENDING'
WHERE order_status = 'PENDING_POS_CONFIRMATION'
AND order_status IS NOT NULL;
...
SELECT order_status, COUNT(*) as count FROM order_status_history GROUP BY order_status ORDER BY count DESC;
