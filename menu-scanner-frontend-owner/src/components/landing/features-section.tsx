          ...
          {LANDING_CONFIG.features.items.map(({ icon, title, description }, i) => (
            <FadeIn key={title} direction="up" delay={i * 80}>
              <Card className="bg-white border-2 border-slate-200 hover:border-primary/30 hover:shadow-xl transition-all duration-300 h-full group relative overflow-hidden">
                {/* Animated bg dot */}
                <div className="absolute top-2 right-2 w-20 h-20 bg-primary/5 rounded-full blur-xl group-hover:scale-150 transition-transform duration-300"></div>

                <CardContent className="p-7 relative z-10">
                  <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center mb-5 group-hover:scale-125 transition-transform text-primary shadow-sm">
                    {iconMap[icon]}
                  </div>
