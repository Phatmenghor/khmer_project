  ...
  return (
    <section id="how-it-works" className="relative py-16">
      ...
      <div className="absolute inset-0 -z-10">
        ...
        <div className="absolute top-1/2 left-0 w-64 h-64 bg-primary/8 rounded-full blur-3xl animate-pulse" style={{animationDuration: "5s"}}></div>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-5">
        <FadeIn direction="up">
          <div className="text-center mb-11">
            ...
            <p className="text-xs text-slate-700 max-w-2xl mx-auto leading-relaxed font-medium">
          ...
          {steps.map(({ number, icon: Icon, title, description }, i) => (
            <FadeIn key={title} direction="up" delay={i * 140}>
              <Card className="relative bg-white border-2 border-slate-200 hover:border-primary/30 hover:shadow-xl transition-all duration-300 h-full group overflow-hidden">
                {/* Animated bg dot */}
                <div className="absolute top-0 right-0 w-20 h-20 bg-primary/5 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-300 -mr-10 -mt-10"></div>

                <CardContent className="p-5 relative z-10">
                  {/* Decorative number */}
                  <div className="text-base font-black text-primary/10 leading-none select-none pointer-events-none mb-1">
                    {number}
                  </div>
                  {/* Icon */}
                  <div className="relative w-11 h-11 rounded bg-primary/10 flex items-center justify-center mb-4 group-hover:scale-125 transition-transform text-primary shadow-sm">
