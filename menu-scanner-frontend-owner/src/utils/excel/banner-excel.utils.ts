/**
 * Banner Excel Utilities for E-Menu Client
 * - downloadBannerTemplate: generates a blank Excel template with required columns
 * - parseBannerImportFile: reads an uploaded Excel and returns row objects
 */
...
function styleHeaderRow(ws: XLSX.WorkSheet, headers: string[]) {
  headers.forEach((_, colIdx) => {
    ...
    const isRequired = colIdx === 0; // Description is required
...
function buildInstructionSheet(wb: XLSX.WorkBook) {
  ...
  data[0] = ["BANNER BATCH IMPORT INSTRUCTIONS", "", ""];
  data[1] = ["", "", ""];
...
export function downloadBannerTemplate() {
  const headers = BANNER_TEMPLATE_COLUMNS.map((c) => c.label);
...
export interface ParsedBannerRow {
  [key: string]: string;
}

export async function parseBannerImportFile(file: File): Promise<{
  headers: string[];
  rows: ParsedBannerRow[];
  errors: string[];
}> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        let sheetName: string | undefined = wb.SheetNames.find((name) => name === "Banners Template");
        if (!sheetName) {
        ...
        const rows: ParsedBannerRow[] = dataRows
          .filter((row) => row.some((cell) => String(cell).trim() !== ""))
          .map((row, rowIdx) => {
            const rowObj: ParsedBannerRow = {};
