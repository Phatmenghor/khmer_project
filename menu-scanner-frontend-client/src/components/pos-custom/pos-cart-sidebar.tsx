...
export function POSCartSidebar({
  ...
  return (
    ...
    >
      {/* Order Header */}
      <div className="flex items-center justify-between px-4 py-3.5 border-b border-border shrink-0 bg-muted/20">
        <div className="flex items-center gap-2.5">
          <ShoppingCart className="w-5 h-5 text-primary" />
          <div>
            <h2 className="font-extrabold text-sm sm:text-base text-foreground">Current Order</h2>
        ...
        <div className="flex items-center gap-2">
          {cartItems.length > 0 && (
            <CustomButton
              ...
              size="sm"
              onClick={clearCart}
              className="text-xs sm:text-sm text-destructive hover:text-destructive hover:bg-destructive/10 h-8 rounded-[6px] gap-1 px-3 font-bold"
            >
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-h-0 h-full">
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground flex-1 px-4 my-auto">
            <div className="p-5 rounded-full bg-muted/40 mb-3">
              <ShoppingCart className="w-10 h-10 text-muted-foreground/40" />
            </div>
            <p className="text-sm sm:text-base font-bold text-foreground">Your order is empty</p>
            <p className="text-xs sm:text-sm mt-1 text-center text-muted-foreground max-w-[240px] leading-relaxed font-medium">
      ...
      <div className="mt-auto border-t border-border bg-card shrink-0 shadow-lg">
        <div className="p-3.5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1 min-w-0">
              <Label className="text-xs sm:text-xs text-muted-foreground uppercase tracking-wider font-black flex items-center gap-1">
          ...
          <div className="rounded-[10px] border border-border/80 bg-muted/20 p-3 space-y-1.5">
            ...
            <div className="flex justify-between items-center text-xs sm:text-sm">
              <span className="text-muted-foreground font-semibold flex items-center gap-1">
                Tax <span className="text-xs bg-muted/70 px-1.5 py-0.2 rounded font-bold border border-border/60">{cartSummary.taxPercentage}%</span>
              </span>
            ...
            <div className="flex justify-between items-center pt-0.5">
              <span className="text-xs sm:text-sm font-black text-foreground">Total Amount</span>
        ...
        <div className="px-3.5 pb-3.5">
          <div className="rounded-[10px] overflow-hidden border border-border shadow-md flex items-stretch bg-background h-12 sm:h-13">
            <CustomButton
              variant="ghost"
              size="sm"
              className="h-full px-4 gap-1.5 text-xs sm:text-sm font-bold border-r border-border hover:bg-muted/60 shrink-0"
              onClick={() => dispatch(setShowOrderDetailsModal(true))}
              title="Add order notes & discount"
            >
              <Tag className="w-4 h-4 text-primary" />
            </CustomButton>
            <div className="flex-1 px-3 py-1 bg-muted/20 min-w-0 flex items-center justify-between">
              <p className="text-xs sm:text-sm text-muted-foreground font-bold truncate">
                {cartSummary.totalQuantity} {cartSummary.totalQuantity === 1 ? "item" : "items"}
              </p>
              <p className="text-base sm:text-lg font-black text-primary leading-none">
                {formatCurrency(cartSummary.finalTotal)}
              </p>
            </div>
            <CustomButton
              ...
              className={cn(
                "flex items-center justify-center px-6 gap-2 transition-all shrink-0 font-black h-full",
                cartItems.length === 0 || isSubmitting
                  ? "bg-muted text-muted-foreground cursor-not-allowed text-xs sm:text-sm"
                  : "bg-primary text-primary-foreground hover:bg-primary/90 active:scale-95 cursor-pointer text-xs sm:text-sm shadow-md"
              )}
            >
              {isSubmitting ? (
