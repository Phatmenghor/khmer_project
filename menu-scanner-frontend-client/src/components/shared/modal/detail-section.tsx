...
export function SectionTitle({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("col-span-2 mt-4 mb-2 first:mt-0 pb-2 border-b border-border flex items-center justify-between w-full", className)}>
      <h3 className="text-xs font-bold text-foreground tracking-tight flex items-center gap-2">
        <span className="w-1.5 h-4 rounded-full bg-primary shrink-0 inline-block shadow-sm" />
        {children}
      </h3>
    </div>
  );
