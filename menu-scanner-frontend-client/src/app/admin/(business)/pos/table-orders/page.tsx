...
function TableOrdersPageInner() {
  ...
  const {
    ...
    baseRoute: ROUTES.ADMIN.TABLE_ORDERS,
    filters: {
  ...
  // Filter strictly for TABLE orders
  const tableOrdersContent = useMemo(() => {
    return orderContent.filter((ord) => {
      const name = ord.customerName || "";
      const phone = ord.customerPhone || "";
      const note = ord.customerNote || "";
      return phone === "Table Service" || name.startsWith("Table ") || note.includes("[Table ");
    });
  }, [orderContent]);
  ...
  const filterConfig = useMemo((): FilterPanelConfig => ({
    ...
    searchPlaceholder: "Search table order...",
    ...
    <div className="flex flex-1 flex-col gap-3 px-1">
      <div className="space-y-3">
        ...
        <DataTableWithPagination
          data={tableOrdersContent}
          columns={columns}
          loading={isLoading}
          emptyMessage="No table orders found"
          getRowKey={(order) => order.id}
          currentPage={filters.pageNo}
          totalElements={tableOrdersContent.length}
          totalPages={Math.ceil(tableOrdersContent.length / globalPageSize) || 1}
