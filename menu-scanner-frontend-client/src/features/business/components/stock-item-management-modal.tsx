                    ...
                    <div className="mt-4 pt-3 border-t">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div className="rounded border border-border bg-muted/50 p-2.5 space-y-1.5">
                          ...
                          <p className="text-sm font-bold text-foreground">${parseFloat(stockItem.price).toFixed(2)}</p>
                          {stockItem.hasPromotion === true && (
                            <div className="pt-1.5 border-t border-border/40 space-y-1 text-xs">
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Type</span>
                                <span className="font-medium">{stockItem.displayPromotionType === "PERCENTAGE" ? "Percentage" : "Fixed"}</span>
                              </div>
                              <div className="flex justify-between">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="font-bold text-green-600">${stockItem.displayPrice?.toFixed(2) || "0.00"}</span>
                              </div>
                              {stockItem.displayPromotionFromDate && stockItem.displayPromotionToDate && (
                                <div className="flex justify-between text-muted-foreground">
                                  <span>Period</span>
                                  <span className="text-right">
                                    {dateTimeFormat(stockItem.displayPromotionFromDate)} → {dateTimeFormat(stockItem.displayPromotionToDate)}
                                  </span>
                                </div>
                              )}
                            </div>
                        ...
                        <div className="rounded border border-border bg-muted/50 p-2.5 space-y-1.5">
                          ...
                          <div className="pt-1.5 border-t border-border/40 flex justify-between text-xs">
                            ...
                            <span className="font-bold text-green-600">
                              ${((form.watch("quantityOnHand") || 0) * (stockItem.hasPromotion === true ? (stockItem.displayPrice || 0) : parseFloat(stockItem.price))).toFixed(2)}
                            </span>
                          </div>
                        </div>
                      </div>
