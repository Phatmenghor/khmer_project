...
export function useDownloadReceipt() {
  ...
  const handleDownloadReceipt = async (
    ...
    try {
      ...
      await saveCanvas(canvas, order.orderNumber, format);
  ...
  const handleDownloadReceiptFromRef = async (
    ...
    try {
      ...
      await saveCanvas(canvas, orderNumber, format);
    } catch (error) {
      console.error("Receipt download error:", error);
      showToast.error("Failed to generate receipt");
    } finally {
  ...
  return { handleDownloadReceipt, handleDownloadReceiptFromRef, downloadingOrderId, isDownloading };
}
