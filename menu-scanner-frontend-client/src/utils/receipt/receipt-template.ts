...
${padRight("Subtotal w/ Add-ons", 35)} ${padLeft(formatPrice(subtotal + customizationTotal), 9)}
${discount > 0 ? `${padRight("Discount", 35)} ${padLeft(`-${formatPrice(discount)}`, 9)}` : ""}
${discount > 0 ? `${padRight("After Discount", 35)} ${padLeft(formatPrice(subtotal + customizationTotal - discount), 9)}` : ""}
